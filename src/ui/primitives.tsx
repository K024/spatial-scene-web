...
/**
 * 图标开关按钮：`active` / `inactive` 两态。
 *
 * 用于「展示内容」这类少量布尔开关（网格地面 / 坐标轴 / 线框 / 双面）。
 * 用按钮而不是 switch：图标本身就是内容提示，一排排开比一行行开关省地方。
 */
export function IconToggle({
  ...
  return (
    <button
      ...
      aria-pressed={active}
      onClick={onClick}
      className={`focus-ring flex items-center justify-center rounded-lg border transition-colors duration-150 ${
 ...
 * 一排图标开关（`展示内容` 的切换条）。
 *
 * `title` 是分段小标题，放在按钮左侧，避免「一排图标不知道是干嘛的」。
 */
