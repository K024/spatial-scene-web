 ...
 *
 * ── 为什么没有 k-means / DBSCAN 迭代 ──
 * 层深放置不需要迭代：六族里除 `errorDriven` 外都是直方图上的**闭式**分位/重加权；
 * `errorDriven` 是 1D 加权 k-means 的 **DP 精确全局解**（不是迭代 Lloyd）。
 * 实测它只影响「层深放哪」这一维（几个百分点，且随 L 翻转），边缘缺口由
 * `refine.ts` 的几何补齐解决。
 *
 * ── 边界为什么是「层深的中点」 ──
