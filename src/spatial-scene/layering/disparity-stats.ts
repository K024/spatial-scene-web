 ...
 * 两者在带权时不同，不要混用。
 *
 * ── 待做：层内**混深**体检（分层的下一步优化目标）──
 * 现在的分层按高斯中心 z 分带，选出的是「深度带的边界」，而每层最终是带内的 relief
 * （`meshing` 出的是逐像素深度网格），**不是**把整层压到一个平面。所以
 * 「点到代表平面的平方误差」这类目标只是代理指标，不等于 GLB 更好。
 * 更贴题的体检量是**层内深度方差**：在累积深度附件上再要一个二阶矩
 * ```
 *   M2 = Σ (w · z²)          （w = 该样本对像素的贡献）
 *   Var(z) = M2 / A − (ED / A)²      （A = Σw；负的小量夹零）
 * ```
 * `Var` 大 = 一个像素里压着多张真实表面（侧视必然裂/糊），这才是"该不该换分层权重/边界"的
 * 判据。**本轮未实现**（要动渲染附件，属新增契约）。
 */
