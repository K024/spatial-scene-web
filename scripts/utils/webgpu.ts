/**
 * node 侧 WebGPU（`webgpu@0.6.0` = Dawn 的 node 绑定）共享工具。
 *
 ...
 *
 * ⚠ 踩过的坑：`webgpu@0.6.0`（Dawn-node）里如果 `GPU` 实例变成垃圾被 GC 掉，
...
export async function createNodeDevice(): Promise<GPUDevice> {
  ...
  //
  // `webgpu@0.6.0`（Dawn 的 node 绑定，Windows + Node 24）里
 ...
 *
 * 为什么强制：`webgpu@0.6.0`（Dawn）在 Windows + Node 24 上，如果进程退出时
